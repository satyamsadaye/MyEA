...
function ScanReports {
    ...
    foreach ($f in $files) {
        ...
        $topStrat = ""; $topRecency = ""; $topAlltime = ""
        ...
        if ($recMatch.Success) { $topStrat = $recMatch.Groups[1].Value.Trim(); $topRecency = $recMatch.Groups[2].Value.Trim(); $topAlltime = $recMatch.Groups[3].Value.Trim() }
        $topPersistStrat = ""; $topPersistScore = ""
        ...
        if ($persistMatch.Success) { $topPersistStrat = $persistMatch.Groups[1].Value.Trim(); $topPersistScore = $persistMatch.Groups[2].Value.Trim() }
        $reportTitle = ""; $titleMatch = [regex]::Match($content, '<h1>([^<]+)</h1>')
        ...
        $generated = ""; $genMatch = [regex]::Match($content, 'Generated:\s*([^<]+)</div>')
        if ($genMatch.Success) { $generated = $genMatch.Groups[1].Value.Trim() }
        $bestStrat = ""; $bestRate = ""
        $noteMatch = [regex]::Match($content, 'Best strategy:\s*<b>([^<]+)</b>\s*\(([^)]+)\)')
        if ($noteMatch.Success) { $bestStrat = $noteMatch.Groups[1].Value.Trim(); $bestRate = $noteMatch.Groups[2].Value.Trim() }
        $top5Codes = @()
        ...
        if ($tableStart -ge 0) {
            ...
            for ($i = 0; $i -lt [Math]::Min(5, $codeMatches.Count); $i++) { $top5Codes += $codeMatches[$i].Groups[1].Value }
        }
        $reports += @{
            Filename = $f.Name; FilePath = $f.Name
            FileTime = $f.LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss")
            Symbol = $symbol; Period = $period; SpawnInterval = $spawnInterval
            RollingMode = $rollingMode; TestStarted = $testStarted; ReportGenerated = $reportGenerated
            TotalStrategies = $totalStrategies; TotalRunsSim = $totalRunsSim; PropMode = $propMode
            Passed = $passed; Blown = $blown; Active = $active; PassRate = $passRate
            Trades = $trades; Signals = $signals; Runs = $runs
            TopStrat = $topStrat; TopRecency = $topRecency; TopAlltime = $topAlltime
            TopPersistStrat = $topPersistStrat; TopPersistScore = $topPersistScore
            ReportTitle = $reportTitle; Generated = $generated; BestStrat = $bestStrat; BestRate = $bestRate
            Top5Codes = $top5Codes; IsForwardTest = $isForwardTest
        }
    }
    return $reports | ConvertTo-Json -Depth 3
}
...
.btn{background:#fff;color:#374151;border:1px solid #d1d5db;padding:8px 14px;border-radius:6px;font-size:14px;cursor:pointer;font-weight:500;text-decoration:none;display:inline-block}
...
<div class="footer">Server running at <code>http://localhost:$port</code> &mdash; <span id="footerDate"></span></div>
</div>
<script>
var reports = [];
...
function loadData(refresh){
    ...
    fetch(url).then(function(r){
        ...
        st.textContent='Error: '+err.message+' — Make sure the server is running (start-server.bat).';
...
Write-Host "Pre-scanning reports..." -NoNewline
$cachedReports = ScanReports
Write-Host " $($cachedReports.Length) reports cached."
...
while ($true) {
    ...
    try {
        if ($localPath -eq "api/reports") {
            $json = $cachedReports
            $buffer = [Text.Encoding]::UTF8.GetBytes($json)
            $response.ContentType = "application/json"
            $response.OutputStream.Write($buffer, 0, $buffer.Length)
        } elseif ($localPath -eq "api/refresh") {
            $cachedReports = ScanReports
            $buffer = [Text.Encoding]::UTF8.GetBytes($cachedReports)
            $response.ContentType = "application/json"
            $response.OutputStream.Write($buffer, 0, $buffer.Length)
        } elseif ($localPath -match "^report/(.+)$") {
            ...
            if (Test-Path $reportPath) {
                ...
                $buffer = [Text.Encoding]::UTF8.GetBytes("Report not found")
                $response.OutputStream.Write($buffer, 0, $buffer.Length)
            }
        } else {
            $buffer = [Text.Encoding]::UTF8.GetBytes($dashboardHtml)
            $response.ContentType = "text/html"
            $response.OutputStream.Write($buffer, 0, $buffer.Length)
        }
    } catch {
